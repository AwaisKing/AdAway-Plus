...
class VpnWatchdog {
    ...
    int getPollTimeout() {
        ...
        if (this.lastPacketReceived < this.lastPacketSent) return POLL_TIMEOUT_WAITING;
     ...
     */
    void setTarget(final InetAddress target) {
        this.checkAlivePacket = new DatagramPacket(new byte[0], 0, 0, target, 53);
    }
     ...
     */
    void initialize(final boolean enabled) {
        if (BuildConfig.DEBUG) Log.d("AWAISKING_APP", "initialize: Initializing watchdog");
        ...
        if (!this.enabled) {
            if (BuildConfig.DEBUG) Log.d("AWAISKING_APP", "initialize: Disabled.");
            return;
        }

        if (this.initPenalty > 0) {
            if (BuildConfig.DEBUG) Log.d("AWAISKING_APP", "init penalty: Sleeping for " + this.initPenalty + "ms…");
            try {
                Thread.sleep(this.initPenalty);
            } catch (final InterruptedException exception) {
                if (BuildConfig.DEBUG) Log.w("AWAISKING_APP", "Failed to wait the initial penalty.");
    ...
    void handleTimeout() throws VpnNetworkException {
        ...
        if (BuildConfig.DEBUG)
            Log.d("AWAISKING_APP", "handleTimeout: Milliseconds elapsed between last receive and sent: "
                    + (int) (this.lastPacketReceived - this.lastPacketSent) + "ms");
        ...
        if (this.lastPacketReceived < this.lastPacketSent && this.lastPacketSent != 0) {
            ...
            if (this.initPenalty > INIT_PENALTY_END) this.initPenalty = INIT_PENALTY_END;
        ...
        if (this.pollTimeout > POLL_TIMEOUT_END) this.pollTimeout = POLL_TIMEOUT_END;
     ...
     */
    void handlePacket(final byte[] packetData) {
        if (!this.enabled) return;
        if (BuildConfig.DEBUG)
            Log.d("AWAISKING_APP", "handlePacket: Received packet of length " + packetData.length);
    ...
    void sendPacket() throws VpnNetworkException {
        if (!this.enabled || this.checkAlivePacket == null) return;

        if (BuildConfig.DEBUG)
            Log.d("AWAISKING_APP", "sendPacket: Sending packet, poll timeout is " + this.pollTimeout);

        try (final DatagramSocket socket = newDatagramSocket()) {
