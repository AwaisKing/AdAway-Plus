package org.pro.adaway.db.entity;
...
import static org.pro.adaway.db.entity.SourceType.FILE;
import static org.pro.adaway.db.entity.SourceType.UNSUPPORTED;
import static org.pro.adaway.db.entity.SourceType.URL;
...
public class HostsSource {
     ...
     * The user source URL.
     */
    public static final String USER_SOURCE_URL = "content://org.pro.adaway/user/hosts";
    ...
    /**
     * Check and return source type from URL
     *
     * @param url The URL to check.
     *
     * @return {@link SourceType} of the URL.
     */
    public static SourceType getUrlType(String url) {
        if (!"https://".equals(url)) {
            if (URLUtil.isHttpUrl(url)) return URL;
            if (URLUtil.isHttpsUrl(url)) return URL;
        }
        if (URLUtil.isContentUrl(url)) return FILE;
        if (url.startsWith("content://")) return FILE;
        return UNSUPPORTED;
    }
    ...
    public static boolean isValidUrl(String url) {
        final SourceType urlType = getUrlType(url);
        return urlType != null && urlType != UNSUPPORTED;
    }
