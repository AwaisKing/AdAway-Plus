    ...
    TOP_LEVEL_DOMAIN {
        ...
        Comparator<LogEntry> comparator() {
            return (entry1, entry2) -> {
                final String[] split1 = entry1.getHost().split("\\.");
                final String[] split2 = entry2.getHost().split("\\.");

                int index1 = split1.length - 1;
                int index2 = split2.length - 1;

                while (index1 >= 0 && index2 >= 0) {
                    final String part1 = split1[index1];
                    final String part2 = split2[index2];

                    final int partCompare = part1.compareTo(part2);
